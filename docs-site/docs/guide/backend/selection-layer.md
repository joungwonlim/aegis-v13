...
**Hard Cut 필터링**으로 위험 종목 제거

```
S1 통과 종목 (1,000개) → S2 Screener → 필터 통과 (~200개)
                         (재무 + 급락 + 과열 + 변동성)
```

### Hard Cut 조건

#### 1. 재무 지표 (Fundamentals)

| 지표 | 조건 | 목적 |
|------|------|------|
| **PER** | 0 이상 50 이하 | 적자/고평가 기업 제외 |
...
| 지표 | 조건 | 목적 |
|------|------|------|
| **1일 수익률** | >= -9% | 당일 급락 종목 제외 |
| **5일 수익률** | >= -18% | 단기 하락 추세 제외 |

#### 3. 과열 제외 (Overheat)

| 지표 | 조건 | 목적 |
|------|------|------|
| **5일 수익률** | 35% 이하 | 단기 급등 과열 종목 제외 |

#### 4. 변동성 제외 (Volatility)

| 지표 | 조건 | 목적 |
|------|------|------|
| **20일 변동성** | 상위 10% 제외 | 고변동성 종목 제외 |

:::tip YAML SSOT
스크리닝 조건은 `backend/config/strategy/korea_equity_v13.yaml`의 `screening` 섹션에서 관리됩니다.
:::
