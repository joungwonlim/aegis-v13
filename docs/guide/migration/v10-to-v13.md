...
| v10 | v13 | 변환 방법 |
|-----|-----|----------|
| `brain.analysis_results` | `signals.factor_scores` | JSON 파싱 후 컬럼 분리 |
...
```sql
-- signals.factor_scores 생성 및 데이터 변환
...
-- v10 brain 결과에서 추출
INSERT INTO signals.factor_scores (date, code, momentum, value, quality, event)
SELECT
    date,
    code,
